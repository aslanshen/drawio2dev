...
elts[0].textContent = mxResources.get('group');
elts[0].setAttribute('title', mxResources.get('group') + ' (' + this.editorUi.actions.get('group').shortcut + ')');
elts[1].textContent = mxResources.get('ungroup');
elts[1].setAttribute('title', mxResources.get('ungroup') + ' (' + this.editorUi.actions.get('ungroup').shortcut + ')');
...
Toolbar.prototype.createTextToolbar = function()
{
	...
	// 已知：所有表格内容都不适用于撤消/重做
// 已知：在 quirks 和 IE8 中单击带有文本（不是图标）的子菜单后失去焦点。这是因为 TD 似乎
// 捕捉这些浏览器中的点击焦点。注意： mxPopupMenu 中图标项目（无文本）的解决方法。
