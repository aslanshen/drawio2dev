...
Graph = function(container, model, renderHint, stylesheet, themes, standalone)
{
	...
	if (typeof mxVertexHandler !== 'undefined')
	{
		...
		{
			return (state.cell == null) ? '#ff3f4c' /* orange */ : mxConstants.GUIDE_COLOR;
		};
		...
		// 通过检查 part=1 是否在样式中并返回父级来处理部分单元格
// 如果父级不在单元格列表中。容器样式用于禁用
// 进入泳道并使用 dropTarget 样式禁用作为放置目标。
// 稍后：处理递归部分
 ...
 * 标志颜色
 */
HoverIcons.prototype.arrowFill = '#6fb1f2';

HoverIcons.prototype.arrowStrock = '#2b75f2';
...
HoverIcons.prototype.triangleUp = (!mxClient.IS_SVG) ? new mxImage(IMAGE_PATH + '/triangle-up.png', 26, 14) :
	...
	'stroke="'+HoverIcons.prototype.arrowStrock+'" fill="' + HoverIcons.prototype.arrowFill + '"/>');
...
HoverIcons.prototype.repaint = function()
{
	...
	if (this.currentState != null)
	{
		...
		if (this.currentState != null &&
			...
			this.graph.isCellConnectable(this.currentState.cell)&&false)
		{
...
Graph.prototype.createTable = function(rowCount, colCount, w, h, title, startSize, tableStyle, rowStyle, cellStyle)
{
	...
	tableStyle = (tableStyle != null) ? tableStyle : 'shape=table;html=0;startSize=' +
		((title != null) ? startSize : '0') + ';container=1;collapsible=0;childLayout=tableLayout;';
	rowStyle = (rowStyle != null) ? rowStyle : 'shape=partialRectangle;html=0;collapsible=0;dropTarget=0;' +
    	'pointerEvents=0;fillColor=none;top=0;left=0;bottom=0;right=0;points=[[0,0.5],[1,0.5]];portConstraint=eastwest;';
	cellStyle = (cellStyle != null) ? cellStyle : 'shape=partialRectangle;html=0;connectable=0;' +
...
Graph.prototype.createCrossFunctionalSwimlane = function(rowCount, colCount, w, h, startSize, tableStyle, rowStyle, firstCellStyle, cellStyle)
{
	...
	var s = 'html=0;collapsible=0;recursiveResize=0;expand=0;pointerEvents=0;';
...
if (typeof mxVertexHandler != 'undefined')
{
	(function()
	{
		...
		/**
		 * 在给定位置添加新标签并返回新单元格。状态是
* 一个可选的边缘状态，用作标签的父级。顶点
* 目前不允许作为州。
		 */
		Graph.prototype.addText = function(x, y, state)
		{
			...
			label.vertex = true;
			var style = 'html=0;align=center;verticalAlign=middle;resizable=0;points=[];';
		...
		Graph.prototype.addForeignObjectWarning = function(canvas, root)
		{
			if (urlParams['svg-warning'] != '0' && root.getElementsByTagName('foreignObject').length > 0)
			{
				...
				// HTML5 导出中隐式命名空间处理的解决方法，IE 添加 NS1 命名空间，因此使用下面的代码
// 在所有 IE 版本中，除了 quirks 模式。已知：将 xlink 命名空间添加到输出中的每个图像标记。
		...
		{
			console.log("获取初始值-->",mxUtils.getValue(state.style, 'html', '0'))
		...
		{
			console.log("获取当前值-->",mxUtils.getValue(state.style, 'html', '0'))
			// if (mxUtils.getValue(state.style, 'html', '0') == '0')
			// {
		...
		/**
		 * 定义 UI 的句柄。在支持的情况下使用数据 URI 来加快加载时间。
		 */
		// TODO: 增加手柄填充  mxClient.IS_SVG -> true
		HoverIcons.prototype.mainHandle = (!mxClient.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-main.png', 17, 17) :
			Graph.createSvgImage(18, 18, '<circle cx="9" cy="9" r="5" stroke="'+HoverIcons.prototype.arrowStrock+'" fill="' + HoverIcons.prototype.arrowFill + '" stroke-width="1"/>');
		HoverIcons.prototype.secondaryHandle = (!mxClient.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-secondary.png', 17, 17) :
			Graph.createSvgImage(16, 16, '<path d="m 8 3 L 13 8 L 8 13 L 3 8 z" stroke="'+HoverIcons.prototype.arrowStrock+'" fill="#fca000"/>');
		HoverIcons.prototype.fixedHandle = (!mxClient.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-fixed.png', 17, 17) :
			Graph.createSvgImage(18, 18, '<circle cx="9" cy="9" r="5" stroke="'+HoverIcons.prototype.arrowStrock+'" fill="' + HoverIcons.prototype.arrowFill + '" stroke-width="1"/><path d="m 7 7 L 11 11 M 7 11 L 11 7" stroke="'+HoverIcons.prototype.arrowStrock+'"/>');
		HoverIcons.prototype.terminalHandle = (!mxClient.IS_SVG) ? new mxImage(IMAGE_PATH + '/handle-terminal.png', 17, 17) :
			Graph.createSvgImage(18, 18, '<circle cx="9" cy="9" r="5" stroke="'+HoverIcons.prototype.arrowStrock+'" fill="' + HoverIcons.prototype.arrowFill + '" stroke-width="1"/><circle cx="9" cy="9" r="2" stroke="'+HoverIcons.prototype.arrowStrock+'" fill="transparent"/>');
		//旋转
