...
EditorUi = function (editor, container, lightbox) {
    ...
    if (!graph.standalone) {
        ...
        // Ctrl/Cmd+Enter 应用编辑值，但在 Safari 中，Ctrl+Enter 创建
// 一个新行（当 Enter 创建一个新段落并且 Shift+Enter 停止时）
        ...
        // Keys that should be ignored if the cell has a value (known: new default for all cells is html=0 so
 ...
 * screen widths <= 640px.
 */
EditorUi.prototype.hsplitPosition = (screen.width <= 640) ? 118 : ((urlParams['sidebar-entries'] != 'large') ? 500 : 240);
...
EditorUi.prototype.getCellsForShapePicker = function (cell) {
    ...
    return [(cell != null) ? this.editor.graph.cloneCell(cell) :
        createVertex('text;html=0;align=center;verticalAlign=middle;resizable=0;points=[];autosize=1;', 40, 20, 'Text'),
        createVertex('html=0;'),
        createVertex('ellipse;html=0;', 120, 80),
        createVertex('rhombus;html=0;', 80, 80),
        createVertex('shape=parallelogram;perimeter=parallelogramPerimeter;html=0;fixedSize=1;'),
        createVertex('shape=trapezoid;perimeter=trapezoidPerimeter;html=0;fixedSize=1;', 120, 60),
        createVertex('shape=hexagon;perimeter=hexagonPerimeter2;html=0;fixedSize=1;', 120, 80),
        createVertex('shape=step;perimeter=stepPerimeter;html=0;fixedSize=1;', 120, 80),
        createVertex('shape=process;html=0;backgroundOutline=1;'),
        createVertex('shape=cube;html=0;boundedLbl=1;backgroundOutline=1;darkOpacity=0.05;darkOpacity2=0.1;', 120, 80),
        createVertex('shape=note;html=0;backgroundOutline=1;darkOpacity=0.05;', 80, 100),
        createVertex('triangle;html=0;', 60, 80),
        createVertex('shape=document;html=0;boundedLbl=1;', 120, 80),
        createVertex('shape=tape;html=0;', 120, 100),
        createVertex('ellipse;shape=cloud;html=0;', 120, 80),
        createVertex('shape=cylinder;html=0;boundedLbl=1;backgroundOutline=1;', 60, 80)];
};
...
EditorUi.prototype.getQueryString = function (name) {
...
EditorUi.prototype.initCanvas = function () {
    // 初始页面布局视图、scrollBuffer 和基于计时器的滚动
    var graph = this.editor.graph;
    ...
    if (this.editor.isChromelessView()) {
        ...
        // 为查看器创建工具栏 - 不要在此处使用 CSS
// 因为这可能会在没有 CSS 的查看器中使用
        if (urlParams['toolbar'] != '0') {
            ...
            // 	addButton(mxUtils.bind(this, function(evt)
            // 	{
            // 		var fid = this.getQueryString("fid");
            // 		graph.openLink("index.html?"+fid, '_self');
...
/**
 * 处理 ctrl+enter 击键以克隆单元格。 禁用ctrl+enter
 */
EditorUi.prototype.ctrlEnter = function () {
    var graph = this.editor.graph;

    if (graph.isEnabled() && false) {
        try {
            var cells = graph.getSelectionCells();
        ...
        try {
            if (Editor.useLocalStorage) {
                ...
                if (xml.length < MAX_REQUEST_SIZE) {
                    ...
                    console.log("xml==" + encodeURIComponent(xml))
...
// 输入按键停止编辑。 N、W、T 保留。
    ...
    if (!this.editor.chromeless || this.editor.editable) {
        ...
        // keyHandler.bindControlKey(13, function() { ui.ctrlEnter(); }); // Ctrl+Enter this.4118
