...
(function () {
    ...
    Menus.prototype.init = function () {
        ...
        editorUi.actions.put('insertTemplate', new Action(mxResources.get('static.template') + '...', function () {
        ...
        //导出图片 xml
        /*editorUi.actions.put('exportXml', new Action(mxResources.get('formatXml') +' 文件', function () {
        ...
        //导出图片 url
        // editorUi.actions.put('exportUrl', new Action(mxResources.get('url')+' 文件', function () {
        ...
        //导出图片 html
        /*editorUi.actions.put('exportHtml', new Action(mxResources.get('formatHtmlEmbedded') +' 文件', function () {
        ...
        //导出图片 pdf
        editorUi.actions.put('exportPdf', new Action(mxResources.get('formatPdf')+' 文件', function () {
            ...
            //     var dlgH = 180;
            //
            //     // console.log(editorUi.pdfPageExport && !noPages) false
            //     // console.log(!noPages)  false
            ...
            //             mxResources.get('transparentBackground'), true,graph.isSelectionEmpty());
            //
            //         //60
            //         dlgH += 30;
            //     }
            ...
            //     // }), null, mxResources.get('export'));
            //     //pdf null null true true true true null null false true
            //     var dlg = new CustomDialog(editorUi, div, mxUtils.bind(this, function () {
                    editorUi.downloadFile('pdf', null, null, null,null,true,true,true,true,null,null,false,true);
            //     }), null, mxResources.get('export'));
        ...
        // 导出图片 svg
        editorUi.actions.put('exportSvg', new Action(mxResources.get('formatSvg')+' 文件', function () {
            ...
            //
            //         if (!isNaN(val) && val > 0) {
            editorUi.exportSvg(1 ,false ,true ,false,true ,false, '0' ,true ,false ,'blank' ,null, 'diagram')
        ...
        // 导出图片  PNG
        editorUi.actions.put('exportPng', new Action(mxResources.get('formatPng')+' 文件', function () {
            // if (editorUi.isExportToCanvas()) {
            //     editorUi.showExportDialog(mxResources.get('image'), false, mxResources.get('export'),
            //         "/page/doc/export-img.html",
            //         mxUtils.bind(this, function (scale, transparentBackground, ignoreSelection, addShadow, editable,
            //                                      embedImages, border, cropImage, currentPage, dummy, grid, keepTheme, exportType) {
            //             var val = parseInt(scale);
            //
            //             if (!isNaN(val) && val > 0) {
             editorUi.exportImage(1, false ,true, false ,true ,'0' ,true ,false ,null ,false, null, null ,'diagram');
            //                 console.log(val, transparentBackground, ignoreSelection,
        ...
        // 导出图片  JPG
        editorUi.actions.put('exportJpg', new Action(mxResources.get('formatJpg')+' 文件' , function () {
            // if (editorUi.isExportToCanvas()) {
            //     editorUi.showExportDialog(mxResources.get('image'), false, mxResources.get('export'),
            //         '',
            //         mxUtils.bind(this, function (scale, transparentBackground, ignoreSelection, addShadow, editable,
            ...
            //
            //             if (!isNaN(val) && val > 0) {
             editorUi.exportImage(1 ,false ,true, false ,false ,'0' ,true ,false ,'jpeg', false, null ,null ,'diagram')
        ...
        editorUi.actions.addAction('usersManual', function () {
            editorUi.openLink('/page/doc/faq.html');
        });

        editorUi.actions.addAction('suggestionsorfeedback', function () {
            $("#suggestionsorfeedback").modal('show');
            $("#sengEmail").click(()=>{
                if(!(/^[A-Za-z0-9\u4e00-\u9fa5]+@[a-zA-Z0-9_-]+(\.[a-zA-Z0-9_-]+)+$/.test($("#email").val()))){
                    toastr.error("邮箱不符合规则");
                    return;
                }
                var reg = /^\s*$/g;
                if (reg.test($("#suggestions").val())) {
                    toastr.error("内容不可为空");
                    return;
                }
                App.address_ajax.ajax('', "/draw/sendfeedback", "post", "json", {email:$("#email").val(),suggestions:$("#suggestions").val()}, function (res) {
                    toastr.success("发送成功");
                    $("#suggestionsorfeedback").modal('hide');
                })
        ...
        //  导出图片 VSDX
        editorUi.actions.put('exportVsdx', new Action(mxResources.get('formatVsdx')+' 文件' , function () {
...
//仅当本地存储可用于以下情况时，才在选项中添加语言菜单
//存储选择。我们不想在旧的浏览器上使用cookies。
//请注意，网址参数lang=XX可用于设置语言
//在较旧的浏览器中。URL参数优先于保存的设置。
        if (false&&(mxClient.IS_CHROMEAPP || isLocalStorage)) {
            console.log("我建立了了")
        ...
        this.put('help', new Menu(mxUtils.bind(this, function (menu, parent) {
            if (!mxClient.IS_CHROMEAPP && editorUi.isOffline()) {
                ...
                this.addMenuItems(menu, ['keyboardShortcuts','-', 'usersManual','suggestionsorfeedback'], parent);
        ...
        editorUi.actions.addAction('shapes...', function () {
            // console.log("点击更多图形",mxClient.IS_CHROMEAPP ,!editorUi.isOffline())
        ...
        this.put('exportAs', new Menu(mxUtils.bind(this, function (menu, parent) {
            ...
            if (!editorUi.isOffline()) {
                menu.addSeparator(parent);
                this.addMenuItem(menu, 'export', parent).firstChild.nextSibling.innerHTML = mxResources.get('advanced')+'文件';
            }
        })));
        ...
        this.put('importFrom', new Menu(mxUtils.bind(this, function (menu, parent) {
            ...
            function pickFileFromService(service) {
                // Drive需要特殊的参数用于库并绕过实时
                service.pickFile(function (id) {
                    if (editorUi.spinner.spin(document.body, mxResources.get('loading'))) {
                        // 注意getFile中的第三个参数表示denyConvert以匹配
                        // 原始DriveClient中的现有签名
                        // 语义稍有不同，但工作方式相同。
            ...
            if (file != null) {
                ...
                var dlg = new FilenameDialog(this.editorUi, filename, mxResources.get('rename'), mxUtils.bind(this, function (title) {
                    if (title != null && title.length > 0 && file != null && title != file.getTitle() &&
                        ...
                        that = this;
                        App.address_ajax.ajax('', "/draw/rename", "post", "json",{
                            "oldName":  LocalFile.prototype.getGrawUrl(), "newName": title
                            },
                            function (data) {
                            if (data.code == 200) {
                                toastr.success(data.msg);
                                LocalFile.prototype.setGrawUrl(data.data);
                                // var url = MXGRAPH_URL + "/index.html?" + window.parent.location.href.split('?')[1];
                                // window.history.pushState(null, null, url)
                                // 如果启用了自动调整大小，则删除旧文件，将新文件保存在保管箱中
                                file.rename(title, mxUtils.bind(this, function (resp) {
                                        that.editorUi.spinner.stop();
                                    }),
                                    mxUtils.bind(this, function (resp) {
                                        that.editorUi.handleError(resp, (resp != null) ? mxResources.get('errorRenamingFile') : null);
                                    }));
                            } else {
                                toastr.error(data.msg)
                                file.rename(oldFileName, mxUtils.bind(this, function (resp) {
                                        that.editorUi.spinner.stop();
                                    }),
                                    mxUtils.bind(this, function (resp) {
                                        that.editorUi.handleError(resp, (resp != null) ? mxResources.get('errorRenamingFile') : null);
                ...
                if (file.constructor == DriveFile) {
                    var dlg = new CreateDialog(editorUi, title, mxUtils.bind(this, function (newTitle, mode) {
                            if (mode == '_blank') {
                                ...
                                // 如果按下创建按钮，则模式为“下载”，表示使用Google云端硬盘
                                if (mode == 'download') {
        ...
        this.editorUi.actions.addAction('shareDrawing',  function () {
            // editorUi.showPublishLinkDialog(mxResources.get('url'), true, null, null,
            //     function (linkTarget, linkColor, allPages, lightbox, editLink, layers) {
            //         console.log("创建分享链接")
            var dlg = new EmbedDialog(editorUi, editorUi.createLink('blank','#4754FF', false, true, '_blank', true ,null ,null, null, true));
            editorUi.showDialog(dlg.container, 440, 240, true, true);
            dlg.init();
                // });
        });
        ...
        this.editorUi.actions.addAction('issue', mxUtils.bind(this, function () {
            try {
                //获取当前文件
                var file = editorUi.getCurrentFile();
                if (file != null) {
        ...
        editorUi.actions.put('insertText', new Action(mxResources.get('text'), function () {
            if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
                graph.startEditingAtCell(insertVertex('文本', 50, 30, 'text;html=0;resizable=0;autosize=1;' +
        ...
        editorUi.actions.put('insertEllipse', new Action(mxResources.get('ellipse'), function () {
            if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
                insertVertex('', 80, 80, 'ellipse;html=0;');
            }
        ...
        editorUi.actions.put('insertRhombus', new Action(mxResources.get('rhombus'), function () {
            if (graph.isEnabled() && !graph.isCellLocked(graph.getDefaultParent())) {
                insertVertex('', 80, 80, 'rhombus;html=0;');
            }
        })).isEnabled = isGraphEnabled;
        ...
        this.put('insert', new Menu(mxUtils.bind(this, function (menu, parent) {
            this.addMenuItems(menu, [ 'insertText', 'insertLink', '-',
                 'insertFreehand', '-', 'insertImage'], parent);
        ...
        this.put('view', new Menu(mxUtils.bind(this, function (menu, parent) {
            ...
            if (isLocalStorage || mxClient.IS_CHROMEAPP) {
                ...
                // if (!editorUi.isOffline() || mxClient.IS_CHROMEAPP || EditorUi.isElectronApp) {
                //     this.addLinkToItem(item, '/page/doc/faq.html#myGraph');
                // }
            }
        ...
        this.put('extras', new Menu(mxUtils.bind(this, function (menu, parent) {
            ...
            this.addMenuItems(menu, ['copyConnect'], parent);
        ...
        this.put('file', new Menu(mxUtils.bind(this, function (menu, parent) {
            if (urlParams['embed'] == '1') {
                ...
                if (file != null && file.constructor == DriveFile) {
                    this.addMenuItems(menu, ['rename', 'makeCopy', 'moveToFolder'], parent);
                } else {
