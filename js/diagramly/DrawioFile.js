...
/**
 * 以毫秒为单位设置自动保存的延迟。默认值为 1500。
 */
DrawioFile.prototype.lastAutosaveRevision = null;

/**
 * 使用自动保存时设置修订之间的延迟。默认为 300000
 * 即 5 分钟。将此设置为 0 以在每次自动保存时创建修订。
 */
...
/**
 * 添加用于自动保存本地更改图表的侦听器。 同步文件
 */
DrawioFile.prototype.synchronizeFile = function (success, error,that,msg) {
    if(this.shadowData!==msg){
        this.mergeFile(msg, success, error, null);
    }else{
        success();
    }
};
...
DrawioFile.prototype.updateFile = function (success, error, abort, shadow) {
    console.log("abort, shadow===",abort, shadow)
    if (abort == null || !abort()) {
...
DrawioFile.prototype.mergeFile = function (file, success, error, diffShadow) {
    ...
    try {
        ...
        var pages = this.ui.getPagesForNode(
            mxUtils.parseXml(file).documentElement)
        ...
        try {
            if (reportError) {
                if (this.errorReportsEnabled) {
                    this.sendErrorReport('合并文件中的错误', null, e);
                } else {
                    var user = this.getCurrentUser();
                    var uid = (user != null) ? user.id : 'unknown';

                    EditorUi.logError('合并文件中的错误', null,
...
DrawioFile.prototype.issue = function () {
    this.ui.issue();
    // this.ui.alert(mxResources.get('sharingAvailable'), null, 380);
};
...
/**
 * Adds the listener for automatically saving the diagram for local changes. 补丁描述符
 */
...
DrawioFile.prototype.handleFileSuccess = function (saved) {
    ...
    //         this.fileChanged();
    //     } else if (saved) {
    //         console.log("this.isTrashed()",this.isTrashed(),this.sync)
    //         if (this.isTrashed()) {
    ...
    //         }
    //     } else {
            this.ui.editor.setStatus('所有的更改已保存');
    //     }
    // }
};
