...
(function () {
     ...
     */
    EditorUi.scratchpadHelpLink = '/page/doc/faq.html#myGraph';
     ...
     */
    EditorUi.prototype.defaultCustomShapeStyle = 'shape=stencil();html=0;';
    ...
    /**
     * 模板操作是否应显示在插入菜单中。模板控制
     */
    EditorUi.prototype.insertTemplateEnabled = false;
    ...
    EditorUi.prototype.createSpinner = function (x, y, size) {
        ...
        var spinner = new Spinner({
            lines: 12, // 要绘制的线数
            length: size, // 每行的长度
            width: Math.round(size / 3), // 线条粗细
            radius: Math.round(size / 2), // 内圆的半径
     ...
     * graph graph对象
     * file localFile
     * url http://localhost:8080/drawio_master/src/main/webapp/index.html
     * forceXml true
     * forceSvg, forceHtml,embeddedCallback,   null
     * ignoreSelection    true
     * compact, uncompressed    null
     */
    EditorUi.prototype.createFileData = function (node, graph, file, url, forceXml, forceSvg, forceHtml,
        ...
        forceXml = (forceXml != null) ? forceXml : false;  //true
        ignoreSelection = (ignoreSelection != null) ? ignoreSelection : true;  //true
    ...
    EditorUi.prototype.getXmlFileData = function (ignoreSelection, currentPage, uncompressed) {
        ...
        // 用于单页导出的生成图模型XML节点   TODO  起始 生成 mxgraph
    ...
    /**
     * 同步当前文件  CurrentFile=>当前文件
     * @param   < true >  false
     */
    EditorUi.prototype.synchronizeCurrentFile = function (forceReload, that, msg) {
        var currentFile = that.getCurrentFile();
        if (currentFile != null) {
            // if (currentFile.savingFile) {
            //
            //     this.handleError({message: mxResources.get('busy')});
            // } else
            //     if (!forceReload && currentFile.invalidChecksum) {
            //     //    无效的校验和
            //     currentFile.handleFileError(null, true);
            // } else
            if (that.spinner.spin(document.body, mxResources.get('updatingDocument'))) {
                // currentFile.clearAutosave();
                that.editor.setStatus('');
                if (forceReload) {
                    //重载文件
                    console.log("重载文件");
                    currentFile.reloadFile(mxUtils.bind(that, function () {
                        currentFile.handleFileSuccess(DrawioFile.SYNC == 'manual');
                    }), mxUtils.bind(that, function (err) {
                        currentFile.handleFileError(err, true);
                    }));
                } else {
                    //同步文件
                    // console.log("同步文件")
                    currentFile.synchronizeFile(mxUtils.bind(that, function () {
                        // console.log("同步==成功")
                        currentFile.handleFileSuccess(DrawioFile.SYNC == 'manual');
                    }), mxUtils.bind(that, function (err) {
                        // console.log("同步==失败")
     ...
     * dorceXml== true
     ...
     ignoreSelection== true
     currentPage== true
     node== undefined compact== undefined file== undefined uncompressed== undefined
     */
    EditorUi.prototype.getFileData = function (forceXml, forceSvg, forceHtml, embeddedCallback, ignoreSelection,
        ...
        //forceXml  ignoreSelection   currentPage
        ignoreSelection = (ignoreSelection != null) ? ignoreSelection : true;  // true
        currentPage = (currentPage != null) ? currentPage : false;  //true
        var graph = this.editor.graph;
        ...
        if (forceSvg || (!forceXml && file != null && /(\.svg)$/i.test(file.getTitle()))) {
            ...
            // 导出第一页的SVG，而通过创建图形可以看到其他页面
            // LATER: 不在第一页时为图形或支持向量机添加缓存
            // //黑暗模式需要刷新，这会破坏所有处理程序
            // LATER: 在这里使用深色主题，绕过刷新
        ...
        //获取到的 mxfile 文件    true  false  undefined    aa2
    ...
    EditorUi.prototype.getHtml = function (node, graph, title, editLink, redirect, ignoreSelection) {
        ...
        // mxfile 的双重压缩不固定，因为它可能导致不兼容
        // 嵌入依赖此格式的客户端。 HTML 文件和导出使用 getHtml2。
        var data = Graph.compress(text);

        // 回退到 URI 编码的 XML 以进行无效压缩
        if (Graph.decompress(data) != text) {
...
//设置或禁用对象的替换文本。需要禁用
//因为PhantomJS似乎忽略了switch语句，把所有文本都画出来。
                ...
                //
                //         //创建下载请求 aa0
                // console.log("ignoreSelection",ignoreSelection)
                var req = this.createDownloadRequest(filename, format, ignoreSelection, null,
...
//仅导出不包含文件数据的图像的当前页面，但对于
//其他格式包含XML或pdf的所有页面，我们需要发送完整的数据并使用
//指定要导出的页面的从/到URL参数。
        ...
        var userPhone = localStorage.getItem("mobile");
        // var userPhone = localStorage.getItem("mobile");
        if (userPhone == null) {
            alert("登录超时");
            window.location.replace(MXGRAPH_URL + "/login.html")
            return;
        }
        // console.log("导出pdf->",data)
        if (this.spinner.spin(document.body, mxResources.get('exporting'))) {
            that=this;
            App.address_ajax.ajax('', "/pubmed/mgconvert/pdf", "post", "json", {
                    'svgText': data
            }, function (res) {
                that.spinner.stop();
                window.location.href = App.address_ajax.get_global_uri() + '/pubmed/' + res.res.uri;
            })
        }


        return null;
        // return new mxXmlRequest("http://218.108.182.182:9000/draw/exportPdf", 'xml=' + encrypted.toString() + '&filename=' +
        //     filename + '&format=' + format +
        //     "&userPhone=" + userPhone);
        // return new mxXmlRequest("/exportPdf", 'format=' + format + range + allPages +
    ...
    EditorUi.prototype.loadLibrary = function (file, expand) {
        ...
        // console.log("EditorUi=2558= init-8", file.getData())

        if (doc.documentElement.nodeName == 'mxlibrary') {
            var images = JSON.parse(mxUtils.getTextContent(doc.documentElement));
            // console.log("init-9 -=-", images)
            _libraryLoaded=this
            App.address_ajax.ajax('', "/draw/getLibrary", "get", "json", "", function (res) {
                // console.log(res);
                // console.log(res.code==200);
                // console.log(res.code=="200");

                if(res.code=="200"){
                    _libraryLoaded.libraryLoaded(file, eval("("+res.data+")"), doc.documentElement.getAttribute('title'), expand);
                }else{
                    _libraryLoaded.libraryLoaded(file, images, doc.documentElement.getAttribute('title'), expand);
                }
            })
                        ...
                        this.importFiles(evt.dataTransfer.files, 0, 0, this.maxImageSize, mxUtils.bind(this, function (data, mimeType, x, y, w, h, img, doneFn, file) {
                            if (data != null && mimeType.substring(0, 6) == 'image/') {
                                ...
                                var doImport = mxUtils.bind(this, function (theData, theMimeType) {
                                    ...
                                    if (theData != null) //尝试将文件解析为 xml（可以是库或 mxfile）。否则会报错
                                    {
    ...
    EditorUi.prototype.showLibraryDialog = function (name, sidebar, images, file, mode) {
        ...
        // console.log("name, sidebar, images, file, mode")
        // console.log(name, sidebar, images, file, mode)
    ...
    /**
     * Transltemplateates this point by the given vector.
     *
     * @param {number} dx X-coordinate of the translation.
     * @param {number} dy Y-coordinate of the translation.
     */
    ...
    EditorUi.prototype.cooperate = function () {
        // let url = window.parent.location.href;
        // console.log("弹出共享页面");
        this.setTheme();
        App.address_ajax.ajax('', "/file/getCoopTeam", "post", "json", {md5: LocalFile.prototype.getGrawFid()}, function (res) {
            if (res.code == 200) {
                var coopHtml = "  <tr>\n" +
                    "            <td>U_" + localStorage.getItem('mobile').substr(7) + "</td>\n" +
                    "                            <td>  </td>\n" +
                    "          <td>归属者</td>\n" +
                    "</tr>";
    ...
    EditorUi.prototype.issue = function () {
        // let url = window.parent.location.href;
        // console.log("弹出发布页面");
        this.setTheme();
        App.address_ajax.ajax('', "/file/editTemplate", "post", "json", {fileMd5: LocalFile.prototype.getGrawFid()}, function (res) {
            if (res.code == 200) {


                $(".temp-img").attr("src", res.data.img);
                $("#tempSum").val(res.data.summary);
                $("#tempTitle").val(res.data.title);
                $("#tempUrl").val(LocalFile.prototype.getGrawUrl());
                $("#tempSureBut").html("修 改");
                $('#tempModal').modal('show');
            } else if (res.code == 500 && res.msg == "没有权限") {
                toastr.error("协作者没有发布权限");
            } else if (res.code == 500 && res.msg == "使用模板所绘制的生物图禁止发布") {
                toastr.error("使用模板所绘制的生物图禁止发布");
            } else {
                $(".temp-img").attr("src", "page/img/up.svg");
                $("#tempSum").val("");
                $("#tempTitle").val(LocalFile.prototype.getTitle());
                $("#tempUrl").val(LocalFile.prototype.getGrawUrl());
                $("#tempSureBut").html("发 布");
                $('#tempModal').modal('show');
            }
        })
    };

    EditorUi.prototype.setTheme = function(){
        if(eval("("+localStorage.getItem(".drawio-config")+")").ui=="dark"){
            $(".modal-content>div").attr("style","color: #fbfbfb;background-color: #151515;");
        }
    }
    ...
    EditorUi.prototype.createImageDataUri = function (canvas, xml, format, dpi) {
        ...
        // 检查有效输出
        if(data=="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAYAAAAfFcSJAAAAAXNSR0IArs4c6QAAAA1JREFUGFdj+P///38ACfsD/QVDRcoAAAAASUVORK5CYII="){
            return data;
        }
    ...
    EditorUi.prototype.doSaveLocalFile = function (data, filename, mimeType, base64Encoded, format, defaultExtension) {
        // console.log(data)
        // console.log(filename, mimeType, base64Encoded, format, defaultExtension)
        // 为XML文件附加.drawio扩展名，不带扩展名
        // 避免浏览器自动附加.xml
        ...
        // 较旧版本的IE（不支持二进制）
            ...
            // 移动Safari中“ WebKitBlobResource错误1”的解决方法
            ...
            // mxXmlRequest.simulate的变通方法不再在PaleMoon中工作
            //如果使用这种方式(即在PaleMoon中导出XML后，PNG导出被破坏)
            //以及iOS上所有浏览器的“WebKitBlobResource error 1”。
            var useDownload = (navigator.userAgent == null ||
                navigator.userAgent.indexOf("PaleMoon/") < 0) &&
                typeof a.download !== 'undefined';

            // Chromium 65跨域锚点下载问题的解决方法
            ...
            if (useDownload || this.isOffline()) {
                ...
                //Graph.compress(data)
                console.log("导出保存的文件Data==",a.href);
    ...
    EditorUi.prototype.saveLocalFile = function (data, filename, mimeType, base64Encoded, format, allowBrowser, allowTab, defaultExtension) {
        ...
        var dlg = new CreateDialog(this, filename, mxUtils.bind(this, function (newTitle, mode) {
                try {
                    // Opens a new window  直接保存至服务器
                    if (mode == '_blank') {
                        console.log("_blank  保存文件")
                        console.log(data);
                ...
                //稍后在做决定
                this.hideDialog();
            }), mxResources.get('saveAs'), mxResources.get('download'), false, allowBrowser, false,
            null, count > 1, rowLimit, data, mimeType, base64Encoded);
        var height = (this.isServices(count)) ? ((count > rowLimit) ? 177 : 270) : 160;
     ...
     * 按给定向量平移此点。
     *
     *最后3个参数是可选的，并且仅当数据可以在客户端上按原样存储时才必须使用
     *无需服务器往返。
     *
     * @param {number} dx X-coordinate of the translation.
     * @param {number} dy Y-coordinate of the translation.
    ...
    EditorUi.prototype.saveRequest = function (filename, format, fn, data, base64Encoded, mimeType, allowTab) {
        ...
        // var height = (this.isServices(count)) ? ((count > 4) ? 177 : 270) : 160;
        var height = 160;
...
//设置或禁用foreignObjects的替代文本。需要禁用
                //因为PhantomJS似乎忽略了switch语句并绘制了所有文本。
                ...
                var doSave = mxUtils.bind(this, function (svgRoot) {
                    ...
                    if (editable) {
                        svgRoot.setAttribute('content', this.getFileData(true, null, null, null, ignoreSelection,
                            currentPage, null, null, null, false).replace("<mxfile","<mxfile host=\"\""));
                    }
...
//		}
    ...
    EditorUi.prototype.exportImage = function (scale, transparentBackground, ignoreSelection, addShadow,
                                               editable, border, noCrop, currentPage, format, grid, dpi, keepTheme, exportType,obj,callback) {
        format = (format != null) ? format : 'png';

        if(exportType=="publish_template"){
            if (obj.thumbImageCache == null) {
                obj.thumbImageCache = new Object();
            }
            let base64_data;
            try {
                obj.editor.exportToCanvas(mxUtils.bind(this, function (canvas) {
                        // this.spinner.stop();
                        try {
                            // console.log("canvas",canvas);
                            base64_data = obj.createImageDataUri(canvas, (editable) ? obj.getFileData(true, null,
                                null, null, true, currentPage) : null, format, dpi)
                            callback(base64_data);
                        } catch (e) {
                            this.handleError(e);
                        }
                    }), null, obj.thumbImageCache, null, mxUtils.bind(this, function (e) {
                        // this.spinner.stop();
                        this.handleError(e);
                    }), null, true, scale || 1, transparentBackground, addShadow,
        ...
        if (this.spinner.spin(document.body, mxResources.get('exporting'))) {
            ...
            //true 1 1 false true null null "0" true false null "diagram"
            // console.log("ignoreSelection, scale , 1, transparentBackground, addShadow,null, null, border, noCrop, grid, keepTheme, exportType")
            // console.log(ignoreSelection, scale , 1, transparentBackground, addShadow,null, null, border, noCrop, grid, keepTheme, exportType)
            try {
                this.editor.exportToCanvas(mxUtils.bind(this, function (canvas) {
                        this.spinner.stop();
    ...
    EditorUi.prototype.insertAsPreText = function (text, x, y) {
        ...
        try {
            cell = graph.insertVertex(null, null, '<pre>' + text + '</pre>',
                x, y, 1, 1, 'text;html=0;align=left;verticalAlign=top;');
                ...
                this.loadImage(text, mxUtils.bind(this, function (img) {
                    ...
                    try {
                        cell = graph.insertVertex(graph.getDefaultParent(), null, text,
                            graph.snap(dx), graph.snap(dy), 1, 1, 'text;' + ((html) ? 'html=0;' : ''));
    ...
    EditorUi.prototype.init = function () {
        ...
        if (!this.editor.chromeless || this.editor.editable) {
            ...
            // this.keyHandler.bindAction(77, true, 'editGeometry', true); // Ctrl+Shift+M  this.10701
            this.keyHandler.bindAction(88, true, 'insertText', true); // Ctrl+Shift+X
            // this.keyHandler.bindAction(75, true, 'insertRectangle'); // Ctrl+K
            // this.keyHandler.bindAction(75, true, 'insertEllipse', false); // Ctrl+Shift+K
        ...
        // console.log("mxgraph==初始化 init-2")
            ...
            if (urlParams['proto'] == 'json') {
                ...
                try {
                    if (data == null) {
                        ...
                        return;
                    } else if (data.action == 'static.template') {
                        this.spinner.stop();
                        ...
                        var dlg = new NewDialog(this, false, data.templatesOnly ? false : data.callback != null,
                            ...
                            mxUtils.bind(this, function (url, info, name) {
                                ...
                                parent.postMessage(JSON.stringify({
                                    event: 'static.template', docUrl: url, info: info,
                                    name: name
                                }), '*');
                            }), null, null,
    ...
    if (window.ExportDialog != null) {
        ...
        ExportDialog.exportFile = function (editorUi, name, format, bg, s, b, dpi, grid) {
            // console.log("弹出高级框")
            App.address_ajax.ajax('', "/user/getUserInfo", "get", "json", "", function (res) {
                // console.log(res)
                if (res.datas.isVip == 1) {
                    ...
                    if (format == 'xml') {
                        ...
                        if (data.length <= MAX_REQUEST_SIZE && w * h < MAX_AREA) {
                            ...
                            toastr.error(mxResources.get('drawingTooLarge'));
                        }
                    }
                } else {
                    toastr.error("会员可导出高清图片")
                    return;
                }
            })
