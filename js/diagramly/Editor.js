...
(function () {
     ...
     */
    Editor.prototype.appName = 'BioVisArt';
    ...
    Editor.prototype.diagramFileTypes = null;
    ...
    Editor.commonEdgeProperties = [
        ...
        {name: 'strokeOpacity', dispName: '描边不透明度', type: 'int', min: 0, max: 100, defVal: 100},
    ...
    Editor.commonVertexProperties = [
        ...
        {name: 'strokeOpacity', dispName: '描边不透明度', type: 'int', min: 0, max: 100, defVal: 100},
        {
            ...
            enumList: [{val: 'visible', dispName: '可见'}, {val: 'hidden', dispName: 'Hidden'}, {
                val: 'block',
                dispName: 'Block'
            },
                {val: 'fill', dispName: '填'}, {val: 'width', dispName: '宽度'}]
        },
        {name: 'noLabel', dispName: '隐藏标签', type: 'bool', defVal: false},
        {name: 'labelPadding', dispName: '标签填充', type: 'float', defVal: 0},
        {
            name: 'direction', dispName: '方向', type: 'enum', defVal: 'east',
            enumList: [{val: 'north', dispName: '北'}, {val: 'east', dispName: '东'}, {
                val: 'south',
                dispName: '南'
            }, {val: 'west', dispName: '西'}]
        },
        {
            name: 'portConstraint', dispName: '约束', type: 'enum', defVal: 'none',
            enumList: [{val: 'none', dispName: '没有任何'}, {val: 'north', dispName: '北'}, {
                val: 'east',
                dispName: '东'
            }, {val: 'south', dispName: '南'}, {val: 'west', dispName: '西'}]
        },
        {name: 'portConstraintRotation', dispName: '旋转约束', type: 'bool', defVal: false},
        {
            name: 'connectable', dispName: '可连接', type: 'bool', getDefaultValue: function (state, format) {
                var cell = (state.vertices.length == 1 && state.edges.length == 0) ? state.vertices[0] : null;
                var graph = format.editorUi.editor.graph;
        ...
        {name: 'allowArrows', dispName: '允许箭头', type: 'bool', defVal: true},
        {name: 'snapToPoint', dispName: '捕捉到点', type: 'bool', defVal: false},
        {
            name: 'perimeter', dispName: '周长', defVal: 'none', type: 'enum',
            enumList: [{val: 'none', dispName: 'None'},
                ...
                {val: 'hexagonPerimeter2', dispName: '六边形'}, {val: 'lifelinePerimeter', dispName: '生命线'},
                {val: 'orthogonalPerimeter', dispName: '正交'}, {val: 'backbonePerimeter', dispName: '支柱'},
                {val: 'calloutPerimeter', dispName: 'Callout'}, {
                    val: 'parallelogramPerimeter',
                    dispName: '平行四边形'
                },
                {val: 'trapezoidPerimeter', dispName: '梯形'}, {val: 'stepPerimeter', dispName: '步'}]
        },
        {name: 'fixDash', dispName: '固定破折号', type: 'bool', defVal: false},
        {name: 'autosize', dispName: '自动尺寸', type: 'bool', defVal: false},
        {
            name: 'container', dispName: '容器', type: 'bool', defVal: false, isVisible: function (state, format) {
                return state.vertices.length == 1 && state.edges.length == 0;
            }
        },
        {
            name: 'dropTarget', dispName: '放下目标', type: 'bool', getDefaultValue: function (state, format) {
                var cell = (state.vertices.length == 1 && state.edges.length == 0) ? state.vertices[0] : null;
                var graph = format.editorUi.editor.graph;
        ...
        {
            name: 'collapsible', dispName: '可折叠', type: 'bool', getDefaultValue: function (state, format) {
                var cell = (state.vertices.length == 1 && state.edges.length == 0) ? state.vertices[0] : null;
                var graph = format.editorUi.editor.graph;
        ...
        {name: 'expand', dispName: '扩大', type: 'bool', defVal: true},
        {
            name: 'part', dispName: '部分', type: 'bool', defVal: false, isVisible: function (state, format) {
        ...
        {name: 'editable', dispName: '可编辑', type: 'bool', defVal: true},
        {name: 'metaEdit', dispName: '编辑对话框', type: 'bool', defVal: false},
        {name: 'backgroundOutline', dispName: '背景大纲', type: 'bool', defVal: false},
        {name: 'movable', dispName: '活动', type: 'bool', defVal: true},
        {
            name: 'movableLabel',
            dispName: '可移动标签',
            type: 'bool',
        ...
        {name: 'resizable', dispName: '可调整大小', type: 'bool', defVal: true},
        {name: 'resizeWidth', dispName: '调整宽度', type: 'bool', defVal: false},
        {name: 'resizeHeight', dispName: '调整高度', type: 'bool', defVal: false},
        {name: 'rotatable', dispName: '可旋转', type: 'bool', defVal: true},
        {name: 'cloneable', dispName: '可克隆', type: 'bool', defVal: true},
        {name: 'deletable', dispName: '可删除', type: 'bool', defVal: true},
        {name: 'treeFolding', dispName: '树折叠', type: 'bool', defVal: false},
        {name: 'treeMoving', dispName: '树移动', type: 'bool', defVal: false},
        {
            name: 'pointerEvents',
            dispName: '指针事件',
            type: 'bool',
    ...
    Editor.defaultCsvValue = '##\n' +
        ...
        '#\n' +
        '# style: label;image=%image%;html=0;rounded=1;fillColor=%fill%;strokeColor=%stroke%;\n' +
        '#\n' +
        '## Parent style for nodes with child nodes (placeholders are replaced once).\n' +
        '#\n' +
        '# parentstyle: swimlane;html=0;childLayout=stackLayout;horizontal=1;horizontalStack=0;resizeParent=1;resizeLast=0;collapsible=1;\n' +
        '#\n' +
    ...
    (function () {
        /**
         * 添加 handJiggle 样式（jiggle=n 设置抖动）
         */
    ...
    Editor.configure = function (config, untrusted) {
        if (config != null) {
            ...
            StyleFormatPanel.prototype.defaultColorSchemes = confidefaultColorSchemesg || StyleFormatPanel.prototype.defaultColorSchemes;
    ...
    /**
     * 将 SVG 输出中的所有图像转换为数据 URI 以便立即呈现
     */
     ...
     * @param callback
     * @param width   null
     * @param imageCache  图像缓存  new Object()
     * @param background   null
     * @param error   null
     * @param limitHeight 极限高度 null
     * @param ignoreSelection 忽略重选  true
     * @param scale 规模  true
     * @param transparentBackground 透明背景  false
     * @param addShadow 添加阴影 null
     * @param converter 转换器  null
     * @param graph 图表  null
     * @param border  0
     * @param noCrop 不用修剪  true
     * @param grid 格子  false
     * @param keepTheme 保持主题  null
     * @param exportType 导出类型 diagram
     */
    Editor.prototype.exportToCanvas = function (callback, width, imageCache, background, error, limitHeight,
        ...
        // console.log(" width, imageCache, background, error, limitHeight,ignoreSelection, scale, transparentBackground, addShadow, converter, graph, border, noCrop, grid,keepTheme, exportType")
        //
        // console.log( width, imageCache, background, error, limitHeight,
        //     ignoreSelection, scale, transparentBackground, addShadow, converter, graph, border, noCrop, grid,
        //     keepTheme, exportType);
        try {
                        ...
                        img.onload = mxUtils.bind(this, function () {
                            try {
                                ...
                                function drawImage() {
                                    ...
                                    // console.log(" callback(canvas)",canvas)
                                    // 首次导出时 Safari 中损坏的数据 URI 图像的解决方法
                                    if (mxClient.IS_SF) {
                                        window.setTimeout(function () {
                                            ctx.drawImage(img, 0, 0);
                                            callback(canvas);
                                ...
                                if (grid) {
                                    ...
                                    view.scale = 1; //临时重置比例以生成未缩放的网格图像，然后对其进行缩放
    ...
    if (window.StyleFormatPanel != null) {
        ...
        StyleFormatPanel.prototype.findCommonProperties = function (cell, properties, addAll) {
            ...
            //侧边栏-设置样式编辑属性
            if (state != null && state.shape != null && false) {
                //为所有形状添加通用属性
        ...
        StyleFormatPanel.prototype.init = function () {
            ...
            this.container.appendChild(this.addCellGraph(this.createPanel()));
        ...
        // 创建小图预览面板
        StyleFormatPanel.prototype.addCellGraph = function (div) {
            var ui = this.editorUi;
            var graph = ui.editor.graph;
            var look = document.createElement('div');
            look.style.whiteSpace = 'nowrap';
            look.style.position = 'relative';
            look.style.textAlign = 'center';

            look.style.paddingLeft = '2px';
            look.style.paddingRight = '2px';
            look.style.paddingLeft = '2px';
            look.style.paddingBottom = '2px';
            look.style.background = 'withe';
            look.style.height = '100px';
            look.style.width = 'white';
            div.appendChild(look)

            return div;
        }
        var cellGraph = mxUtils.bind(this, function () {})
        /**
...
//updateScheme函数用于更新颜色样式，它会根据传入的颜色设置图形的填充色、边框色和文字颜色。
                        ...
                        try {
                            ...
                            for (var i = 0; i < cells.length; i++) {
                                ...
                                if (colorset != null) {
                                    ...
                                    style = mxUtils.setStyle(style, mxConstants.STYLE_TEST, '#fff2cc' ||
                                        mxUtils.getValue(defaults, mxConstants.STYLE_TEST, null));

                                    // style = mxUtils.setStyle(style, mxConstants.STYLE_HIGHTLIGH, colorset['gradient'] ||
                                    //     mxUtils.getValue(defaults, mxConstants.HIGHTLIGH, null));

                                    // ALT shift ctrl 控制单独变换填充等颜色
        ...
        StyleFormatPanel.prototype.addEditOps = function (div) {
            ...
            //侧边栏-编辑数据
            if (graph.getSelectionCount() == 1 && state != null && state.shape != null && state.shape.stencil != null && false) {
    ...
    /**
     * 指定通过动态 JS 加载的特殊库。添加案例
     * 无法从包名中计算出文件名。这
     * 此映射的标准方案是 stencils/packagename.xml。如果有
     * 是多个 XML 文件、任何 JS 文件或文件名中的任何异常或
     * 包含该文件的目录，则必须在此处添加一个条目并
     * 在 EmbedServlet2 中加载形状以工作。
     */
    // 需要避免 mockup.xml 出现 404，因为 mxgraph.mockup.anchor 的命名不包含
    // 按钮，即使它是在 mxMockupButtons.js 文件中定义的。这只能修复
    // 现有形状的别名或基本名称的别名，但这本质上是相同的。
        ...
        function preview(print) {
            ...
            if (pv == null) {
                ...
                if (pv.mathEnabled) {
                    ...
                    // 在 MathJax 完成渲染时添加异步打印
                    // 通过在 math-print.js 中检查的全局变量
                    // 避免生成不安全的内联脚本或将 SHA 添加到 CSP
                    if (print) {
