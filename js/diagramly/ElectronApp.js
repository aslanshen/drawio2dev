...
window.DRAWIO_BASE_URL = '.'; //阻止访问在线网站，因为它是不允许的
FeedbackDialog.feedbackUrl = '';

//禁用 JS 的 eval（使用 shape.min.js）
...
(function()
{
	...
	Menus.prototype.init = function()
	{
		...
		// 替换文件菜单以将openFrom菜单替换为open并重命名downloadAs进行导出
	...
	*/
	...
	mxElectronRequest.prototype.getText = function()
	{
		...
		{
			console.log("另一个 createDownloadRequest ")
			var graph = this.editor.graph;
			var bounds = graph.getGraphBounds();
			
//仅导出当前页面以获取不包含文件数据但包含
//包含XML的其他格式或包含所有页面的pdf，我们需要发送完整的数据并使用
// from / to URL参数，以指定要导出的页面。
	...
	EditorUi.prototype.saveData = function(filename, format, data, mimeType, base64Encoded)
	{
		...
		// Spinner.stop是异步的，因此我们必须异步调用保存对话框// //给Spinner一些时间来停止旋转
